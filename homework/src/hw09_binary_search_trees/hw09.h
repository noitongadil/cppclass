...
public:
        ...
        , m_size(0)
    {
        ...
        if (not std::is_sorted(local_arr.begin(), local_arr.end()))
        {
...
private:
    ...
    /**
     * @brief Helper for validating BST subtree.
     *
     * @param node Pointer to node to validate.
     * @param min Pointer to the lower bound.
     * @param max Pointer to the upper bound.
     * @return True if subtree is a valid BST within (min, max), false otherwise.
     */
    ...
    /**
     * @brief Print helper that prints the nodes with padded spaces on the left.
     *
     * @param current Pointer to the node to start printing from.
     * @param level Indentation level used to align output.
     */
    ...
    /**
     * @brief Delete all nodes in a tree using post-order traversal.
     *
     * @param current Pointer to the node to start deleting from. 
    ...
    /**
     * @brief Insert nodes into this tree by traversing another tree in prefix order.
     *
     * @param current Pointer to a node in the other tree insert into this tree.
     */
    ...
    /**
     * @brief Build a balanced BST by inserting the middle elements of a sorted array.
     *
    ...
    /**
     * @brief Compare two trees for structural and value equality using prefix traversal.
     *
     * @param self_curr Root of the tree in this.
     * @param other_curr Root of the tree in other.
    ...
    {
        if (self_curr == nullptr && other_curr == nullptr)
        {
            return true;
        }
    ...
    FRIEND_TEST(BasicBinarySearchTreeTest, DefaultConstructor);
    FRIEND_TEST(BasicBinarySearchTreeTest, GetSizeWithMutating);
    FRIEND_TEST(BasicBinarySearchTreeTest, MoveConstructor);
    FRIEND_TEST(BasicBinarySearchTreeTest, CopyConstructor);
    FRIEND_TEST(BasicBinarySearchTreeTest, ArrayConstructor);
    FRIEND_TEST(BasicBinarySearchTreeTest, TreeValidation);
    FRIEND_TEST(BasicBinarySearchTreeTest, Remove);
    FRIEND_TEST(BinarySearchTreeTest, ACID);
};
} // namespace cppclass
